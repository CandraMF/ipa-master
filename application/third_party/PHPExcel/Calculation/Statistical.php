...
class PHPExcel_Calculation_Statistical {
	 ...
	 * References:
	 * <ol>
	 * <li>W. J. Cody and K. E. Hillstrom, 'Chebyshev Approxandriions for the Natural
	 *	 Logarithm of the Gamma Function,' Math. Comp. 21, 1967, pp. 198-203.</li>
	 * <li>K. E. Hillstrom, ANL/AMD Program ANLC366S, DGAMMA/DLGAMA, May, 1969.</li>
	 * <li>Hart, Et. Al., Computer Approxandriions, Wiley and sons, New York, 1968.</li>
	 * </ol>
	 * </p>
	 * <p>
	 ...
	 * The program uses rational functions that theoretically approxandrie LOG(GAMMA)
	 * to at least 18 significant decimal digits. The approxandriion for X > 12 is from
	 * reference 3, while approxandriions for X < 12.0 are similar to those in reference
	...
	// Rough estandrie of the fourth root of logGamma_xBig
	...
	private static function _inverse_ncdf($p) {
		//	Inverse ncdf approxandriion by Peter J. Acklam, implementation adapted to
		...
		//	Coefficients in rational approxandriions
		...
		if (0 < $p && $p < $p_low) {
			//	Rational approxandriion for lower region.
			$q = sqrt(-2 * log($p));
			return ((((($c[1] * $q + $c[2]) * $q + $c[3]) * $q + $c[4]) * $q + $c[5]) * $q + $c[6]) /
					(((($d[1] * $q + $d[2]) * $q + $d[3]) * $q + $d[4]) * $q + 1);
		} elseif ($p_low <= $p && $p <= $p_high) {
			//	Rational approxandriion for central region.
			$q = $p - 0.5;
			$r = $q * $q;
			return ((((($a[1] * $r + $a[2]) * $r + $a[3]) * $r + $a[4]) * $r + $a[5]) * $r + $a[6]) * $q /
				   ((((($b[1] * $r + $b[2]) * $r + $b[3]) * $r + $b[4]) * $r + $b[5]) * $r + 1);
		} elseif ($p_high < $p && $p < 1) {
			//	Rational approxandriion for upper region.
			$q = sqrt(-2 * log(1 - $p));
			return -((((($c[1] * $q + $c[2]) * $q + $c[3]) * $q + $c[4]) * $q + $c[5]) * $q + $c[6]) /
					 (((($d[1] * $q + $d[2]) * $q + $d[3]) * $q + $d[4]) * $q + 1);
		}
	...
	private static function _inverse_ncdf2($prob) {
		//	Approxandriion of inverse standard normal CDF developed by
...
//			$CumPGuessMinus1 = $CumPGuess - $PGuess;
	 ...
	 * STDEV
	 *
	 * Estandries standard deviation based on a sample. The standard deviation is a measure of how
	 ...
	 * STDEVA
	 *
	 * Estandries standard deviation based on a sample, including numbers, text, and logical values
	 *
	 * Excel Function:
	 ...
	 * VARFunc
	 *
	 * Estandries variance based on a sample.
	 *
	 * Excel Function:
	 *		VAR(value1[,value2[, ...]])
	 *
	 ...
	 * VARA
	 *
	 * Estandries variance based on a sample, including numbers, text, and logical values
	 *
	 * Excel Function:
	 *		VARA(value1[,value2[, ...]])
	 *
