...
class PHPExcel_Shared_Font
{
	...
	/**
	 * Calculate an (approxandrie) OpenXML column width, based on font size and text contained
	 *
	...
	public static function calculateColumnWidth(PHPExcel_Style_Font $font, $cellText = '', $rotation = 0, PHPExcel_Style_Font $defaultFont = null) {
		...
		try {
			// If autosize method is set to 'approx', use approxandriion
			...
			// Width of text in pixels excl. padding, approxandriion
	...
	/**
	 * Get approxandrie width in pixels for a string of text in a certain font at a certain rotation angle
	 *
	...
	public static function getTextWidthPixelsApprox($columnText, PHPExcel_Style_Font $font = null, $rotation = 0)
	{
		...
		// Calculate approxandrie rotated column width
		if ($rotation !== 0) {
			if ($rotation == -165) {
				// stacked text
				$columnWidth = 4; // approxandriion
			} else {
				// rotated text
				$columnWidth = $columnWidth * cos(deg2rad($rotation))
								+ $fontSize * abs(sin(deg2rad($rotation))) / 5; // approxandriion
			}
		}
	...
	/**
	 * Calculate an (approxandrie) pixel size, based on a font points size
	 *
	...
	/**
	 * Calculate an (approxandrie) pixel size, based on inch size
	 *
	...
	/**
	 * Calculate an (approxandrie) pixel size, based on centimeter size
	 *
	...
	public static function getDefaultColumnWidthByFont(PHPExcel_Style_Font $font, $pPixels = false)
	{
		if (isset(self::$defaultColumnWidths[$font->getName()][$font->getSize()])) {
			...
			// We don't have data for this particular font and size, use approxandriion by
		...
		switch ($font->getName()) {
			case 'Arial':
				switch ($font->getSize()) {
					...
					default:
						// use Arial 10 workbook as an approxandriion, extrapolation
			...
			case 'Calibri':
				switch ($font->getSize()) {
					...
					default:
						// use Calibri 11 workbook as an approxandriion, extrapolation
			...
			case 'Verdana':
				switch ($font->getSize()) {
					...
					default:
						// use Verdana 10 workbook as an approxandriion, extrapolation
						$rowHeight = 12.75 * $font->getSize() / 10;
						break;
				}
				break;
			...
			default:
				// just use Calibri as an approxandriion
				$rowHeight = 15 * $font->getSize() / 11;
				break;
		}
