<?php
	...
	$KDCPCL=$this->input->get('KDCPCL');
	if($Action=="hapus"){
		//echo "delete from dbsipd_".$_tahun.".t_kegiatan_cpcl where KDCPCL='{$KDCPCL}' ";
		$this->db->query("delete from dbsipd_".$_tahun.".t_kegiatan_cpcl where KDCPCL='{$KDCPCL}' ");
	}
 ...
 <table class="table table-bordered table-striped with-check">
  ...
  <tbody>		   
	<?php			
		$JENIS_CPCL="3";
		$i=1;$KDTAHAP=$_tahap;
		$qry=$this->db->query("SELECT a.KDKEGUNIT, e.KDPRGRM, a.KDDEWAN ,a.KDCPCL, f.NMPRGRM, a.KEGIATAN_PRIORITAS, a.SASARAN_KEGIATAN, concat(a.LOKASI,' ', c.NMDESA,' ' , b.NMUNIT) as LOKASI, concat(a.TARGET, ' ',a.SATUAN) VOLUME, a.PAGU, a.PENANGGUNG_JAWAB, a.FOTO, a.TGLVALID, g.NMDEWAN, bb.NMUNIT FROM dbsipd_".$_tahun.".t_kegiatan_cpcl AS a 
		...
		foreach ($qry->result() as $row){	
			...
			<tr>	
				...
				<td>".$row->NMDEWAN."</td>
				<td>".$row->NMUNIT."</td>
				<td>[<a href='".base_url('login/cetak/hapusbl/?Action=hapus&KDCPCL='.$row->KDCPCL)."'>Hapus</a>]</td>
