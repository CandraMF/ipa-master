<?php
	...
	switch($Aksi){
		case "edit":
			$row = new stdClass();	
			$row=$this->m_action->ambilData("select * from dbsipd_".$_tahun.".mrekb where MTGKEY='".$id."'");	
			
			$readonly=" readonly ";
		break;
		case "hapus":	
			$qry="delete from dbsipd_".$_tahun.".mrekb where MTGKEY='".$id."'";
			$this->db->query($qry);		
			$this->m_action->logData($qry, "DELETE", "dbsipd_".$_tahun.".mrekb","MTGKEY", $id);
			header("location:".base_url('login/pages/'.$Pr));
		break;
		default:
			if(empty($Aksi)){
				$row=$this->m_action->KosongkanData("dbsipd_".$_tahun.".mrekb");		
			}
	}
...
<script>	
	$( document ).ready(function() {		
		...
		$('#btnsimpan').click(function(){	
			...
			$.post("<?=base_url('daftar/'.$Pr.'_rekb/simpan')?>",dataPost,
			function(data){	
				...
				setTimeout(function(){ parent.location="<?=base_url('login/pages/'.$Pr.'?tab=tab3')?>"; }, 1000);
			});
		});
	});	
</script>