...
<input type="hidden" name="AksiData" value="<?=@$AksiData?>">
<div class="row-fluid">
			...
			<?php			
				...
				foreach ($qry->result() as $row){	
					...
					<tr>					
						...
						<td>
							<center ".$valid.">
								".$this->andri->cmbUmum("cmbAksi","",$ArrCmbUmum,"data-id='".@$row->KDRANCKEG."' class='cmbAksi' style='width:80px;'")."
							</center><br>
...
<script type="text/javascript">	
	$( document ).ready(function() {	
		...
		$('.cmbAksi').change(function(){	
			...
			switch(cmbData) {			 
			  case "Upload KAK":
				  parent.location="<?=base_url('login/pages/'.$Pr.'/upload-kak/edit/')?>"+KDRANCKEG;
			  break;
			  case "Edit":
				  parent.location="<?=base_url('login/pages/'.$Pr.'/kegiatan/edit/')?>"+KDRANCKEG;
			  break;
			  case "Delete":
				  parent.location="<?=base_url('login/pages/'.$Pr.'/kegiatan/hapus/')?>"+KDRANCKEG;
			  break;
			}   
		});
	});
